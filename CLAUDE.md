...
- **OG image edge functions must stay under Vercel's 1MB limit — never import the `validEIPs`/`validRIPs`/`validCAIPs` arrays into any `runtime = "edge"` code** (`validEIPs` alone is ~365KB). Instead they read tiny precomputed files, both regenerated from `getProposalListItems`/the valid-* arrays so they can't drift, and both chained into `bun run eip`:
  - **List images** (`app/{eips,ercs,rips,caips}/opengraph-image.tsx`) render only a count → `utils/proposalListOgImage.tsx` reads `data/proposal-counts.json` via `@/data/proposalCounts`. Regenerate: `bun run gen-counts` (`scripts/genProposalCounts.ts`).
  - **Per-proposal image** (`app/api/og/route.tsx`) needs title/status/isERC for any one proposal → reads the slim `data/og-index.json` (~117KB) via `@/data/ogIndex`. Regenerate: `bun run gen-og-index` (`scripts/genOgIndex.ts`).
